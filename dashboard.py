...
TRADE_WEBHOOK_URL = "https://discordapp.com/api/webhooks/1502106028286808165/a9kWWOB3eTSAEQZVHvUgtzOHr5VS93Msz25X7SxJgBfkiC3zVknKh-6sdTSlpvr2nvc0"

NEWS_WEBHOOK_URL = "https://discordapp.com/api/webhooks/1502122150549651517/8SnlZjUsEDcGMjDdFCPtdEua4xNHWbVe7i0sIt1gDHBG1SCuvWrIixy0_cCDBm5LHR_X"

SUMMARY_WEBHOOK_URL = "https://discordapp.com/api/webhooks/1502135565745782866/Q6HtrJ3LI8gQletouf8hy0C2DJJi_5iVzE_4IHTWL44UYYmugeIgoWreXdOeoemB7dkh"
...
DISCORD_WEBHOOK_URL = "https://discordapp.com/api/webhooks/1502106028286808165/a9kWWOB3eTSAEQZVHvUgtzOHr5VS93Msz25X7SxJgBfkiC3zVknKh-6sdTSlpvr2nvc0"
...
for position in st.session_state.portfolio:
    current_data = yf.Ticker(position["Ticker"]).history(period="1d")
...
if st.button("Reset Paper Account"):
    ...
    pd.DataFrame(st.session_state.portfolio).to_csv(
        PORTFOLIO_FILE,
        index=False
    )

    pd.DataFrame(st.session_state.trade_history).to_csv(
        TRADE_HISTORY_FILE,
        index=False
    )
...
if st.session_state.portfolio:
    ...
    largest_position = allocation_df.loc[
        allocation_df["Allocation %"].idxmax()
    ]
...
if not sell_trades.empty:
    ...
    # TRADE WIN/LOSS CHART

    trade_chart_df = sell_trades.copy()

    trade_chart_df["Trade #"] = range(
        1,
        len(trade_chart_df) + 1
    )
...
if signal_filter != "ALL":
    ...
    filtered_watchlist = (
        filtered_watchlist[
            filtered_watchlist["AI Signal"]
            == signal_filter
        ]
    )

# BULK BUY FILTERED WATCHLIST
...
if st.button("Send Daily Market Summary"):
    ...
    st.success(
        "Daily market summary sent to Discord!"
    )
...
if st.button("Run Backtest"):
    ...
    backtest_stock = yf.Ticker(backtest_ticker)

    backtest_data = backtest_stock.history(period="1y")
    ...
    buy_trades = trade_log_df[
        trade_log_df["Action"] == "BUY"
    ]

    sell_trades_chart = trade_log_df[
        trade_log_df["Action"] == "SELL"
    ]
    ...
    for strategy in strategies:
        ...
        test_data = backtest_stock.history(period="1y")
        ...
        if test_shares > 0:
            ...
            test_final_value += (
                test_shares * test_data["Close"].iloc[-1]
            )

        test_return = (
            (test_final_value - 10000) / 10000
        ) * 100
